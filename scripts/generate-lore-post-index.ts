...
async function buildLorePostIndex(): Promise<IndexedLorePostFile[]> {
  ...
  return posts
    ...
    .map(({ filename, explicitPublishDate }) => ({
      filename,
      explicitPublishDate,
    }));
}
...
function renderIndexFile(posts: IndexedLorePostFile[]): string {
  const renderedEntries = posts
    .map((post) => {
      const lines = ['  {', `    filename: '${post.filename}',`];

      if (post.explicitPublishDate) {
        lines.push(`    explicitPublishDate: '${post.explicitPublishDate}',`);
      }

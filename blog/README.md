...
- If `public/blog/unassigned/` has a fitting image available, claim one for the new post by moving it out of `unassigned/` and renaming it to match the post date (e.g., `public/blog/YYYY-MM-DD.png`). Then set `cover_image: /blog/YYYY-MM-DD.png`.
- Use `public/blog/placeholder.png` (`cover_image: /blog/placeholder.png`) when there is no fitting image.
- Do not reuse another date's cover image path for a new post date.
